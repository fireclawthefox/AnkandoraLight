...
class DBotPlayerAI(DistributedObjectAI):
    ...
    def move(self):
        ...
        if self.room.gameType == RoomGlobals.GAMETYPE_NORMAL:
            ...
            self.allPaths = self.room.boardAI.getAllPathsToFieldName(self.currentField, "EndField_{}".format(self.botId+1))


            #... there is something wrong in this alternative path calculations

            deletablePathsIdx = []

            for pathList in self.allPaths.copy():
                if pathList[0] == self.lastField:
                    print("REMOVE PATH LEADING BACK")
                    self.allPaths.remove(pathList)
    ...
    def moveNext(self):
        ...
        if field.cost > self.room.currentRoll:
            ...
            # the best path is not possible with our roll, check other paths
            for altPath in self.allPaths:
                # check if this alternative path is good
                if altPath[0].cost <= self.room.currentRoll:
                    field = altPath[0]
                    self.path = altPath[1:]
                    self.allPaths = []
            ...
            if not foundPossiblePath:
                # we haven't found an alternative way
                self.processAndEndTurn(self.currentField)
                return Task.done
        ...
        self.lastField = self.currentField
        self.currentField = None
        self.room.setMultiplePlayersOnField(self.lastField)
